...
class ExtratorTexto:
    ...
    def criar_pdf(self, texto, arquivo_imagem, nome_pdf):
        try:
            ...
            # Adicionar texto à primeira página do PDF
            c.setFont("Helvetica", 12)
            y_pos = 750  # Posição inicial do texto na página
            for linha in texto.split('\n'):
                c.drawString(50, y_pos, linha)
                y_pos -= 14  # Ajuste para a próxima linha

            # Adicionar imagem à segunda página do PDF
            c.showPage()
            c.drawImage(arquivo_imagem, 0, 0, width=letter[0], height=letter[1])
            c.showPage()
            c.save()

            # messagebox.showinfo("Concluído", f"Arquivo PDF '{nome_pdf}' criado com sucesso.")
        except Exception as e:
